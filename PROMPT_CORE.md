...
>
> **⚠️ Color de banda duplicado en 6 mapas — cambiar TODOS juntos:** `BANDA_COLORS` (`render_helpers.py`) · `_AR_BANDA_C`, el `getBanda` interno, el mapa `'Exitosa':{...}` ~L3059, `ar3_bandColors` (`js_override.js`) · `banda_colors` del bloque `_bk` (`assemble_unified.py`). Reemplazar por **par completo** (`bg`+`fg` juntos) — los hexes pálidos viejos (#E1F5EE, #FCE4F1…) se comparten con pills/accent-soft, un replace de hex suelto rompe esas cosas.

| Banda | bg | fg | barra severidad |
|---|---|---|---|
| Exitosa | `#1A6B4A` | `#FFFFFF` | `#1A6B4A` |
| Aceptable | `#FBBF24` | `#5C3A00` | `#FCD34D` |
...
- **Pills · dos líneas (W24-layout):**
  - **Primera línea (selector de dimensión** Destino/Corp/Hotel/Channel · País/Destino/Corp/Hotel**):** color de **SECCIÓN** — CR **violet** (`#5C469C` texto/borde · activa relleno `#EDE8F7`) · RND **magenta** (`#EA0074` · activa relleno `#FCE4F1`). Activa = relleno claro + **MAYÚSCULA**; inactiva = outline + title-case. **NUNCA verde acá.** Estático en `render_cr_p1.py`/`render_rnd_p1.py` (`_PILL_ACTIVE`/`_PILL_INACT`), dinámico en `kpi_setView` (assemble, `sec_col`/`sec_bg`).
