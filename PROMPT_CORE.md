...
**Hoja Maestra:** una fila por Hotel × Canasta (4 canastas como filas). Columnas: dimensiones + métricas + **Banda ND/Ef** + **Banda CV** (ambas coloreadas). Canasta es columna filtrable. RND incluye País; CR incluye Channel. IPM eliminado de RND.
...
✅ **Strip RND W25** — banda NoDispo correcta (3,43% = Aceptable, no Súper Crítica) + IPM eliminado del strip · 27-06-2026.
✅ **H1 W25** — "Disponibilidad & Conectividades" · 27-06-2026.
✅ **Footer W25** — dentro del shell + 2 filas (Destinos MX/US/CALA + Cuentas Global/Estratégicas) + labels Disponibilidad/Conectividades · 27-06-2026.
...
**Última actualización:** W26-carryover-canastas · 29-06-2026 (**Carryover rediseñado + aclaración de arquitectura editorial por canasta** — **(1) Carryover:** `build_carryover` (`editorial_engine.py`) ahora DERIVA el área responsable por hotel vía helper `_carry_area(corp, banda)`: cuenta gestionada (corp ∈ `GLOBAL_ACCOUNTS['corps']` ∪ `CUENTAS_ESTRATEGICAS['corps']`) → "Supply Comercial / Supply Optimization" · independiente + banda Súper Crítica → "Supply Optimization / TPS" · resto → "Supply Optimization". Eliminado `'estado':'Persistente'`; `ref_lbl` "Persiste desde W{prev}" → "Desde W{prev}". Item: `{t,a,sub,o}`. Render en `demo_js_main.js` (contenedor `w22-co`): badge de área AZUL `#2C5282`/`#E3ECF5`/border-left `#2C5282` (antes gris), label COMPLETO (no `.split('/')[0]`), sin badge de estado. **(2) Aclaración crítica de arquitectura (NO requirió cambio de código):** el editorial RE/PA **por canasta** ES el panel dinámico con chips (`w22-re-list`/`w22-pg`/`w22-co`, `w22_setC`→`w22_update`), que lee `CR_D[canasta]`/`RND_D[canasta]`. Esa data se genera **por scope** con `build_editorial_cr/rnd(D, scope=key)` en `render_cr_p2.py`/`render_rnd_p2.py`, así que cualquier cambio a `build_findings_*`/`build_action_plan_*`/`build_carryover` se propaga AUTOMÁTICAMENTE a global + b2c + op + cug. **NO existen secciones estáticas por canasta**: `render_canasta_block` (en `render_*_p3.py`) está definida pero **nunca se llama** desde W21 (Part 3 = 0 chars · "canastas en JS"); los 19 `.canasta-block` del HTML son solo selectores CSS muertos. Verificado por jsdom: al cambiar de chip cada canasta re-renderiza scopeada con findings de cohorte ("Hoteles con Performance de 0%" CR / "Tasa de No Dispo Crítica" RND) + drilldowns Corps/Destinos/Hoteles. ⚠️ No perder tiempo editando `render_canasta_block`/`template_resumen.py` para "replicar por canasta" — es código muerto.)
