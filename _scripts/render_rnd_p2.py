    ...
    for i, r in df.iterrows():
        ...
        for c in cols_def:
            ...
            if c.get('key') == 'hotel':
                ...
                sub = r.get('CorpName','')
                row_cells += (f'<div style="text-align:left;max-width:320px;">'
...
def render_demanda_nc():
    ...
    cols = [
        {'key':'hotel','label':'Hotel','width':'auto','fmt':lambda r:'','align':'left'},
        {'key':'trafico','label':'Tráfico','width':'auto','fmt':lambda r:fmt_big(r['Trafico'])},
...
def _render_dim_table_rnd(df, dim_col, dim_label, start_idx=0, sb_id=None):
    ...
    for i, r in df.iterrows():
        ...
        cells = (f'<div style="overflow:hidden;text-align:left;max-width:280px;"><span style="font-size:11px;font-weight:600;color:var(--ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:block;">{row_idx+1}. {truncate(raw_label,36)}</span></div>'
                 f'{badge_cell}'
...
def render_bloque_hoteles():
    ...
    cols_dnc = [
        {'key':'hotel','label':'Hotel','width':'auto','fmt':lambda r:'','align':'left'},
        {'key':'bnd','label':'Severity','width':'auto','fmt':lambda r:'','align':'left'},
        {'key':'trafico','label':'Tráfico','width':'auto','fmt':lambda r:fmt_big(r['Trafico'])},
        {'key':'pctnd','label':'%NoDispo','width':'auto','fmt':lambda r:fmt_pct2(r['%NoDispo'])},
        {'key':'wownd','label':'WoW','width':'auto','fmt':lambda r:_fmt_wow_rnd(r.get('NoDispo_WoW_pp'), mejora_si_negativo=True)},
        {'key':'rpm','label':'IPM','width':'auto','fmt':lambda r:'$'+fmt_num2(max(r.get('RPM',r.get('IPM',0)),0))},
        {'key':'wowipm','label':'WoW','width':'auto','fmt':lambda r:_fmt_wow_rnd((r['IPM_WoW_pp']/r['IPM_W18']*100) if r.get('IPM_WoW_pp') is not None and r.get('IPM_W18',0)>0 else None, mejora_si_negativo=False)},
    ]
