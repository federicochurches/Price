    ...
    for i, r in df.iterrows():
        ...
        for c in cols_def:
            ...
            if c.get('key') == 'hotel':
                ...
                sub = r.get('CorpName','')
                row_cells += (f'<div style="text-align:left;min-width:0;overflow:hidden;max-width:280px;">'
...
def _render_dim_table_rnd(df, dim_col, dim_label, start_idx=0, sb_id=None):
    ...
    for i, r in df.iterrows():
        ...
        cells = (f'<div style="overflow:hidden;text-align:left;min-width:0;max-width:220px;"><span style="font-size:11px;font-weight:600;color:var(--ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:block;">{row_idx+1}. {truncate(raw_label,36)}</span></div>'
                 f'{badge_cell}'
