...
function _parseDrill(html, col){
 if(!html||!html.trim())return '';
 var tmp=document.createElement('div');
 tmp.innerHTML=html;
 var COLORS={'Corporativos':{c:'#5C469C',bg:'#EDE8F7'},'Corps':{c:'#5C469C',bg:'#EDE8F7'},
             'Destinos':{c:'#185FA5',bg:'#E3EEF9'},'Hoteles':{c:'#EA0074',bg:'#FCE4F1'}};
 function renderCol(sec){
  var sc=COLORS[sec.label]||{c:col,bg:'#F0EBE2'};
  var rows=sec.items.slice(0,5).map(function(it){
   return '<div style="display:flex;justify-content:space-between;align-items:baseline;padding:3px 7px;border-bottom:1px solid #EDE9E1;gap:6px;">'
    +'<span style="font-size:9.5px;font-weight:600;color:var(--ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;">'+it[0]+'<\/span>'
    +(it[1]?'<span style="font-size:9px;color:var(--ink-muted);flex-shrink:0;white-space:nowrap;">'+it[1]+'<\/span>':'')
    +'<\/div>';
  }).join('');
  return '<div style="border:1px solid #EDE9E1;border-radius:3px;overflow:hidden;min-width:0;">'
   +'<div style="padding:4px 7px;border-bottom:1px solid #EDE9E1;background:'+sc.bg+';">'
   +'<span style="font-size:8px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:'+sc.c+';">'+sec.label+'<\/span><\/div>'
   +rows+'<\/div>';
 }
 var sections=[];
 tmp.querySelectorAll('[style*="border-left"]').forEach(function(el){
  var labelEl=el.querySelector('[style*="uppercase"]');
  if(!labelEl)return;
  var label=labelEl.textContent.trim();
  var items=[];
  el.querySelectorAll('strong').forEach(function(s){
   var name=s.textContent.replace('…','').replace('...','').trim();
  ...
  if(items.length)sections.push({label:label,items:items});
 });
 if(!sections.length)return html;
 var top=sections.filter(function(s){return s.label!=='Hoteles';});
 var bot=sections.filter(function(s){return s.label==='Hoteles';});
 var out='<div style="display:flex;flex-direction:column;gap:5px;">';
 if(top.length){
  out+='<div style="display:grid;grid-template-columns:'+(top.length>1?'1fr 1fr':'1fr')+';gap:5px;">';
  out+=top.map(renderCol).join('');
  out+='<\/div>';
 }
 if(bot.length){
  out+='<div style="display:grid;grid-template-columns:1fr;gap:5px;">';
  out+=bot.map(renderCol).join('');
  out+='<\/div>';
 }
 return out+'<\/div>';
}
...
function w22_update(){
 ...
 var pg=g('w22-pg');
 if(pg){var planItems=d.plan||[];
 pg.innerHTML=planItems.map(function(p,i){
  var last=i===planItems.length-1;
  var borderB=last?'none':'1px dashed var(--rule-soft)';
