...
"""
import pickle, zipfile, shutil, re
import sys, os
...
def _read_inventory_kpis(week_num):
    """Lee card-total/card-pp/card-gap (netnew) del INVENTORY_WNN.html real de
    esa semana. None si el archivo no existe o no tiene esas cards — nunca
    inventa un valor viejo (antes esta función no existía y los KPIs de
    Inventory del Hub quedaban hardcodeados a los de W26 para siempre).
    Mismo mecanismo de lectura que ya usa render_mail_v3.py.
    """
    path = Path(f'inventory/week-{week_num}/INVENTORY_W{week_num}.html')
    if not path.exists():
        return None
    html_txt = path.read_text(encoding='utf-8', errors='ignore')
    def _card(cid):
        m = re.search(r'id="' + cid + r'"[^>]*>\s*([\d.,]+)', html_txt)
        return m.group(1).replace('.', '').replace(',', '') if m else None
    total, pp, netnew = _card('card-total'), _card('card-pp'), _card('card-gap')
    if not pp:
        return None
    return {
        'total': int(total) if total else None,
        'pp': int(pp),
        'netnew': int(netnew) if netnew else 0,
    }
...
def build_index():
    ...
    # Colores WoW pre-calculados. inverse=True para métricas donde subir es
    # peor (ej. %NoDispo) — sin esto, un empeoramiento se pintaba en verde.
    def _wc(v, inverse=False):
        good = (v < 0) if inverse else (v >= 0)
        return ("#1A6B4A","#E1F5EE") if good else ("#FF3B30","#FFE5E3")
    def _wb(v,s,inverse=False): fg,bg=_wc(v,inverse); return f'<div style="font-size:9px;font-weight:700;color:{fg};background:{bg};padding:1px 6px;border-radius:10px;display:inline-block;margin-top:2px;">{s}</div>'
    wow_ef  = _wb(cr_ef_wow,  cr_ef_wow_str)
    wow_cv  = _wb(cr_cv_wow,  cr_cv_wow_str)
    wow_nd  = _wb(rnd_wow,    rnd_wow_str, inverse=True)
    ...
    # Inventory KPIs — leídos en vivo del INVENTORY_WNN.html real de esta semana
    # y de la anterior (antes: literales hardcodeados de W26, congelados para
    # siempre; ver W28-code-review). Target de Producto Propio: 70.000.
    INV_TARGET = 70_000
    _inv_cur  = _read_inventory_kpis(WEEK)
    _inv_prev = _read_inventory_kpis(WEEK_PREV)
    ...
    if _inv_cur:
        ...
        # Si no hay HTML de la semana anterior, aproximar el PP previo
        # restando el netnew de esta semana — degrada con gracia, no rompe.
        if _inv_prev:
            _pp_prev    = _inv_prev['pp']
            _prev_total = _inv_prev['total'] if _inv_prev['total'] else _inv_total
        else:
            _pp_prev    = max(_inv_pp - _netnew, 0)
            _prev_total = _inv_total

        _inv_n_d    = _inv_total - _prev_total
        _pp_pct     = round(_netnew / _pp_prev * 100, 2) if _pp_prev else 0
        _pp_pct_str = f'+{_pp_pct:.2f}%' if _pp_pct >= 0 else f'{_pp_pct:.2f}%'
        wow_inv_pp  = _wb(_netnew, _pp_pct_str)

        _av_prev    = round(_pp_prev / INV_TARGET * 100, 2)
        _av_curr    = round(_inv_pp  / INV_TARGET * 100, 2)
...
<div id="hub">
  ...
  <div class="hub-grid" style="margin-bottom:20px;">
    ...
    <div class="rpt-card card-active" onclick="location.href='inventory/{WEEK_STR}/INVENTORY_W{WEEK}.html'" style="cursor:pointer;">
      <div class="rpt-card-top">
        ...
        <div style="font-size:16px;font-weight:700;margin-bottom:2px;color:var(--ink);">Inventario</div>
      </div>
      <div class="rpt-pills">
        <div class="rpt-pills-left">
          <span class="rpt-pills-label">Historial</span>
  ...
  <!-- ── Card Descargas unificada ── -->
  <div class="hub-grid" style="margin-bottom:20px;grid-template-columns:1fr;">
    <div class="rpt-card card-active" data-no-link="1">
      <div class="rpt-card-top">
        <span class="rpt-accent" style="background:#EDEAE4;color:#6B6861;">Activo</span>
        <div style="font-size:16px;font-weight:700;margin-bottom:2px;color:var(--ink);">Centro de Descargas</div>
        ...
        <div style="margin-top:12px;border:1px solid var(--rule);border-radius:3px;overflow:hidden;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1px 1fr 1fr;border-bottom:1px solid var(--rule);">
            ...
            <div style="background:var(--rule);"></div>
            <a href="https://raw.githubusercontent.com/federicochurches/Price/main/rates-nodispo/week-{WEEK}/regional/Analisis_RND_Mexico_W{WEEK}.xlsx" onclick="event.stopPropagation()" style="display:block;padding:6px 10px;font-size:10px;font-weight:700;color:#EA0074;text-decoration:none;text-align:center;">↓ Excel</a>
            <a href="https://raw.githubusercontent.com/federicochurches/Price/main/checkrates/week-{WEEK}/regional/Analisis_CR_Mexico_W{WEEK}.xlsx"  onclick="event.stopPropagation()" style="display:block;padding:6px 10px;font-size:10px;font-weight:700;color:#5C469C;text-decoration:none;text-align:center;border-left:1px solid var(--rule);">↓ Excel</a>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1px 1fr 1fr;border-bottom:1px solid var(--rule);">
            <div style="padding:6px 10px;font-size:10px;font-weight:500;color:var(--ink);">Destinos US</div>
            <div style="background:var(--rule);"></div>
            <a href="https://raw.githubusercontent.com/federicochurches/Price/main/rates-nodispo/week-{WEEK}/regional/Analisis_RND_US_W{WEEK}.xlsx" onclick="event.stopPropagation()" style="display:block;padding:6px 10px;font-size:10px;font-weight:700;color:#EA0074;text-decoration:none;text-align:center;">↓ Excel</a>
            <a href="https://raw.githubusercontent.com/federicochurches/Price/main/checkrates/week-{WEEK}/regional/Analisis_CR_US_W{WEEK}.xlsx"  onclick="event.stopPropagation()" style="display:block;padding:6px 10px;font-size:10px;font-weight:700;color:#5C469C;text-decoration:none;text-align:center;border-left:1px solid var(--rule);">↓ Excel</a>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1px 1fr 1fr;border-bottom:1px solid var(--rule);">
            <div style="padding:6px 10px;font-size:10px;font-weight:500;color:var(--ink);">CALA</div>
            <div style="background:var(--rule);"></div>
            <a href="https://raw.githubusercontent.com/federicochurches/Price/main/rates-nodispo/week-{WEEK}/regional/Analisis_RND_CALA_W{WEEK}.xlsx" onclick="event.stopPropagation()" style="display:block;padding:6px 10px;font-size:10px;font-weight:700;color:#EA0074;text-decoration:none;text-align:center;">↓ Excel</a>
            <a href="https://raw.githubusercontent.com/federicochurches/Price/main/checkrates/week-{WEEK}/regional/Analisis_CR_CALA_W{WEEK}.xlsx"  onclick="event.stopPropagation()" style="display:block;padding:6px 10px;font-size:10px;font-weight:700;color:#5C469C;text-decoration:none;text-align:center;border-left:1px solid var(--rule);">↓ Excel</a>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1px 1fr 1fr;border-bottom:1px solid var(--rule);">
            <div style="padding:6px 10px;font-size:10px;font-weight:500;color:var(--ink);">Global Accounts</div>
            <div style="background:var(--rule);"></div>
            <a href="https://raw.githubusercontent.com/federicochurches/Price/main/rates-nodispo/week-{WEEK}/accounts/Analisis_RND_GlobalAccounts_W{WEEK}.xlsx" onclick="event.stopPropagation()" style="display:block;padding:6px 10px;font-size:10px;font-weight:700;color:#EA0074;text-decoration:none;text-align:center;">↓ Excel</a>
            <a href="https://raw.githubusercontent.com/federicochurches/Price/main/checkrates/week-{WEEK}/accounts/Analisis_CR_GlobalAccounts_W{WEEK}.xlsx"  onclick="event.stopPropagation()" style="display:block;padding:6px 10px;font-size:10px;font-weight:700;color:#5C469C;text-decoration:none;text-align:center;border-left:1px solid var(--rule);">↓ Excel</a>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1px 1fr 1fr;">
            <div style="padding:6px 10px;font-size:10px;font-weight:500;color:var(--ink);">Cuentas Estratégicas</div>
            <div style="background:var(--rule);"></div>
            <a href="https://raw.githubusercontent.com/federicochurches/Price/main/rates-nodispo/week-{WEEK}/accounts/Analisis_RND_Estrategicas_W{WEEK}.xlsx" onclick="event.stopPropagation()" style="display:block;padding:6px 10px;font-size:10px;font-weight:700;color:#EA0074;text-decoration:none;text-align:center;">↓ Excel</a>
            <a href="https://raw.githubusercontent.com/federicochurches/Price/main/checkrates/week-{WEEK}/accounts/Analisis_CR_Estrategicas_W{WEEK}.xlsx"  onclick="event.stopPropagation()" style="display:block;padding:6px 10px;font-size:10px;font-weight:700;color:#5C469C;text-decoration:none;text-align:center;border-left:1px solid var(--rule);">↓ Excel</a>
          </div>
        </div>
      </div>
      <div class="rpt-pills">
