...
/* CR_MEMBERSHIP: construido desde CR_HOTEL_POOL (W25+).
   Proporciona corp→dest y dest→corp para CR (EF/CV) que es distinto de RND_MEMBERSHIP.
   BK también usa esta membresía (mismo contexto de corporativos). */
(function() {
  if (typeof CR_HOTEL_POOL === 'undefined') return;
  var cd = {}, dc = {};
  CR_HOTEL_POOL.forEach(function(h) {
    var corp = h[1], dest = h[2];
    if (!corp || !dest) return;
    if (!cd[corp]) cd[corp] = [];
    if (cd[corp].indexOf(dest) < 0) cd[corp].push(dest);
    if (!dc[dest]) dc[dest] = [];
...
function _handleKpiCardHistClick(e, row, kpiRows) {
  ...
  if (!_isHotelRow && cardKey === 'bk' && typeof _kpiView !== 'undefined' && _kpiView['bk'] === 'hotel') {
    _isHotelRow = true;
  }
  ...
  if (cardId === 'kpicard-ar1' || cardId === 'kpicard-ar2') {
    ...
    if (_arV === 'hotel') {
      ...
      if (_arHWas) {
        ...
        /* Reset KPI value + badge + WoW box a los datos Global de la canasta activa —
           _arPillRender NO toca el valor/badge (solo re-renderiza filas), hay que
           forzar ar_updateKPIs para volver a pintar el estado Global real. */
        if (typeof ar_updateKPIs === 'function') ar_updateKPIs();
        else if (typeof _arPillRender === 'function') _arPillRender(_arN);
      } else {
        ...
        if (!isNaN(_arHWc)) {
          ...
          /* Badge de banda: reflejar la banda del HOTEL seleccionado, no la de la canasta Global
             (quedaba stale porque solo se actualizaban kpiVal + wowbox). */
          var _arHMetricName = isCR ? (_arN===1 ? 'eficacia' : 'convrate') : 'nodispo';
          function _arHBandaCalc(val, metric) {
            var pct = val / 100;
            if (metric === 'eficacia') {
              if (pct >= 0.97) return 'Exitosa'; if (pct >= 0.93) return 'Aceptable';
              if (pct >= 0.85) return 'Revisar'; if (pct >= 0.60) return 'Crítica'; return 'Súper Crítica';
            }
            if (metric === 'convrate') {
              if (pct === 0) return 'Sin Conversión'; if (pct < 0.008) return 'Crítica';
              if (pct < 0.015) return 'Revisar'; if (pct <= 0.025) return 'Aceptable'; return 'Exitosa';
            }
            if (pct < 0.03) return 'Exitosa'; if (pct <= 0.05) return 'Aceptable';
            if (pct <= 0.20) return 'Revisar'; if (pct <= 0.60) return 'Crítica'; return 'Súper Crítica';
          }
          var _arHBanda = _arHBandaCalc(_arHWc, _arHMetricName);
          var _arHBc = (typeof _AR_BANDA_C !== 'undefined' && _AR_BANDA_C[_arHBanda]) || {bg:'#F2EEE6', fg:'#5F5E5A'};
          var _arHBadgeEl = document.getElementById('ar'+_arN+'-badge');
          if (_arHBadgeEl) {
            _arHBadgeEl.textContent = _arHBanda;
            _arHBadgeEl.style.background = _arHBc.bg;
            _arHBadgeEl.style.color = _arHBc.fg;
            _arHBadgeEl.style.border = '1px solid ' + _arHBc.fg + '44';
          }
          if (_arHWbEl) {
  ...
  if (isSelected) {
    ...
    _rCids.forEach(function(rcid) {
      ...
      if (_rfn) { /* reset via direct call: re-fetch el y renderPanel con VD global */
        document.dispatchEvent(new CustomEvent('hist-reset', {detail: {cid: rcid}}));
      } else {
        document.dispatchEvent(new CustomEvent('hist-reset', {detail: {cid: rcid}}));
      }
      var _rl = document.getElementById('hist-' + rcid + '-label');
      if (_rl) _rl.textContent = 'Global';
    });
    return;
  }
  ...
  /* Lookup histórico real W18-W(N-1) para rellenar la sparkline con datos corp/hotel */
  var _arHistArr = null;
  /* AR3 (BK AR card) o BK KPI card en vista hotel → BK_HOTEL_HIST por nombre de hotel */
  if ((cardId === 'kpicard-ar3' || cardId === 'kpicard-bk') &&
      typeof window.BK_HOTEL_HIST !== 'undefined' && window.BK_HOTEL_HIST[label]) {
...
/* ── AR card (NoDispo) · cambio de banda (Críticos/Bajo Rend./Sin Conv.) ──
   Espejo simplificado de kpi_setView: muestra el panel de la banda elegida,
   resalta la pill activa (opacidad plena) y opaca las inactivas. La card AR
   es solo-hotel, sin cross-filter de dimensión. */
var _arBand = {nd: 'crit'};
function ar_setBand(card, band, el) {
  ...
  if (panelsEl) {
    panelsEl.querySelectorAll('.ar-'+card+'-panel').forEach(function(p) {
      p.style.setProperty('display', (p.getAttribute('data-band') === band) ? 'block' : 'none', 'important');
    });
  }
}
...
function _kpiPillRender(card) {
  ...
  (function _crossFilterNonHotel(){
    ...
    if (!_panel) return;
    var _N = _kpiNormCF;
    /* CR cards usan CR_MEMBERSHIP (construida desde CR_HOTEL_POOL).
       RND cards usan RND_MEMBERSHIP (construida desde g_hotel RND). */
    var _isCRcard = (card === 'ef' || card === 'cv' || card === 'bk');
    var _MEM = _isCRcard
      ? (typeof CR_MEMBERSHIP !== 'undefined' ? CR_MEMBERSHIP : {corpDest:{}, corpPais:{}})
      : (typeof RND_MEMBERSHIP !== 'undefined' ? RND_MEMBERSHIP : {corpDest:{}, corpPais:{}});
    ...
    /* Corp→Corp self-filter ELIMINADO: al seleccionar un corp en la vista corp,
       la lista de corps sigue visible completa (solo se resalta la fila). */
    ...
    _rows.forEach(function(row){
      ...
      if (activeTab === 'destino') {
        ...
        /* Corp→Corp self-filter eliminado: al seleccionar corp, la lista no se filtra */
      }
      if (_ok) {
        _shown++;
  ...
  if (activeTab === 'hotel') {
    ...
    if (card === 'bk') {
      if (_hasCf && cf.corp) {
        /* BK hotel: solo filtrar por corp (g_hotel no tiene Destino — pendiente W26). */
        var _sh2 = 0, _ex2 = 0;
        ...
        _cfSetupMoreBtn(_hotelCont, _ex2);
      } else {
        /* Sin corp filter: mostrar todos */
...
function _kpiCrossFilterClear(card, type) {
  ...
  /* Limpiar estado del searchbox para que al deseleccionar no quede la lista inmensa.
     _kpiSbPinned mantiene el pin del searchbox — hay que borrarlo al limpiar el CF. */
  if (typeof _kpiSbPinned !== 'undefined') _kpiSbPinned[card] = null;
  var _sbInp = document.getElementById('sb-kpi-' + card);
  if (_sbInp && _sbInp.value) { _sbInp.value = ''; }
  var _sbClear = document.getElementById('sb-kpi-' + card + '-clear');
  if (_sbClear) _sbClear.style.display = 'none';
  /* Limpiar rows que el searchbox haya mostrado/ocultado (sb-search-hit con !important) */
  var _sbPanelEl = document.getElementById('kpi-' + card + '-panels');
  if (_sbPanelEl) {
    _sbPanelEl.querySelectorAll('.sb-search-hit').forEach(function(r) {
...
document.addEventListener('click', function(e) {
  ...
  /* AR1/AR2 hotel view: js_override.js ya maneja highlight + sparkline AR correcto (hcr-ar-ef/cv).
     Si este listener continúa, dispara hist-update a hcr-panel-ef (KPI card) en lugar del AR sparkline,
     pisando el trabajo de js_override. En hotel view, hacer early return aquí. */
  if ((containerId === 'ar1-th' || containerId === 'ar2-th') &&
      typeof _arPillView !== 'undefined') {
    var _arN2 = (containerId === 'ar1-th') ? 1 : 2;
    if ((_arPillView[_arN2] || 'hotel') === 'hotel') return;
  }
...
/* ── Layout C NoDispo: stats renderizadas estáticamente en Python (render_rnd_p1.py) ── */
...
<div class="ar-cards-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;">
  ...
  <div class="kpi-card" id="kpicard-ar1" style="border:1px solid var(--rule);padding:0;border-radius:3px;background:var(--paper);">
    ...
    <div style="padding:12px 16px 0;">
      ...
      <div style="margin-top:4px;display:flex;align-items:flex-start;gap:14px;flex-wrap:wrap;">
        <div>
          <div id="ar-kpi-1" style="font-size:36px;font-weight:700;letter-spacing:-.02em;color:#333132;line-height:1;">—</div>
    ...
    <!-- Filas -->
    <div id="ar1-body" style="display:flex;flex-direction:row;align-items:stretch;">
    <div id="ar1-rows-wrap" style="padding:0 16px 0;flex:1;min-width:0;">
    ...
    <div id="ar1-hist-wrap" style="display:none;padding:12px 14px 16px;flex-shrink:0;width:210px;align-items:center;border-left:1px solid var(--rule-soft);overflow:hidden;">
  ...
  <div class="kpi-card" id="kpicard-ar2" style="border:1px solid var(--rule);padding:0;border-radius:3px;background:var(--paper);">
    ...
    <div style="padding:12px 16px 0;">
      ...
      <div style="margin-top:4px;display:flex;align-items:flex-start;gap:14px;flex-wrap:wrap;">
        <div>
          <div id="ar-kpi-2" style="font-size:36px;font-weight:700;letter-spacing:-.02em;color:#333132;line-height:1;">—</div>
...
<div class="re-pa-grid" style="border-top:1px solid var(--rule);padding-top:48px;margin-bottom:64px;display:grid;grid-template-columns:1fr 1fr;gap:32px;align-items:start;">
